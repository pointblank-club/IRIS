...
class PassSequenceDataset(Dataset):
    ...
    def _tokenize_sequences(self):
        ...
        for sample in self.samples:
            ...
            token_ids.extend(
                [self.pass_vocab.get(p, self.pass_vocab["<unk>"]) for p in seq]
            )
    ...
    def __getitem__(self, idx):
        ...
        return (sample["features_scaled"], sample["sequence_tokens"])
...
def beam_search_decode(
    ...
    feature_vector = np.array(
        [raw_features_dict.get(k, 0.0) for k in feature_keys], dtype=np.float32
    )
