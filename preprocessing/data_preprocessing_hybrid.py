...
def build_vocabularies(data_entries):
    ...
    for entry in data_entries:
        ...
        for key in sorted(entry.keys()):
            if key.startswith("machine_"):
                ...
                if isinstance(value, bool):
                    ...
                    machine_config_parts.append(
                        f"{key.replace('machine_', '')}_{value}"
                    )

        # Create hardware identifier from machine config
        if machine_config_parts:
            hardware = "_".join(
                machine_config_parts[:5]
            )  # Limit to first 5 features to avoid too long names
        else:
        ...
        tagged_machine_passes = clean_and_tag_passes(
            machine_pass_list, hardware_prefix=hardware_prefix
        )
    ...
    print(
        f"Sample passes in vocab: {list(joint_pass_vocab.keys())[4:14]}"
    )  # Show first 10 actual passes
...
def load_and_preprocess_data(
    ...
    program_features_array = (
        np.array(program_features_for_scaler, dtype=np.float32)
        if program_features_for_scaler
        else np.array([], dtype=np.float32).reshape(0, len(feature_keys))
    )
    target_metrics_array = (
        np.array(target_metrics_for_scaler, dtype=np.float32)
        if target_metrics_for_scaler
        else np.array([], dtype=np.float32).reshape(0, len(target_metrics))
    )
    ...
    if scale:
        if feature_scaler is None:
            ...
            scaled_features = (
                feature_scaler.transform(program_features_array)
                if program_features_array.size
                else program_features_array
            )
        print(
            "Program features scaler fitted."
            if program_features_array.size
            else "No program features to scale."
        )

        if target_metric_scaler is None:
            target_metric_scaler = StandardScaler()
            if target_metrics_array.size:
                scaled_target_metrics = target_metric_scaler.fit_transform(
                    target_metrics_array
                )
            else:
                scaled_target_metrics = target_metrics_array
        else:
            scaled_target_metrics = (
                target_metric_scaler.transform(target_metrics_array)
                if target_metrics_array.size
                else target_metrics_array
            )
        print(
            "Target metrics scaler fitted."
            if target_metrics_array.size
            else "No target metrics to scale."
        )
    else:
        scaled_features = program_features_array
        scaled_target_metrics = target_metrics_array
    ...
    for i, entry in enumerate(raw_data_entries):
        ...
        tagged_machine_passes = clean_and_tag_passes(
            machine_passes_list, hardware_prefix=hardware_prefix
        )
        ...
        if len(unified_pass_sequence) == 0:
            print(
                f"Skipping sample due to empty unified_pass_sequence. Original entry: {entry}"
            )
            continue
        ...
        input_token_ids.extend(
            [
                joint_pass_vocab.get(p, joint_pass_vocab["<unk>"])
                for p in unified_pass_sequence
            ]
        )
        ...
        if len(input_token_ids) < max_seq_len:
            input_token_ids.extend(
                [joint_pass_vocab["<pad>"]] * (max_seq_len - len(input_token_ids))
            )
        else:
        ...
        if len(target_token_ids) < max_seq_len:
            target_token_ids.extend(
                [joint_pass_vocab["<pad>"]] * (max_seq_len - len(target_token_ids))
            )
        else:
            target_token_ids = target_token_ids[:max_seq_len]
            target_token_ids[-1] = joint_pass_vocab[
                "<eos>"
        ...
        # Encode hardware
        hardware_id = hardware_vocab.get(
            hardware, hardware_vocab.get("default_hardware", hardware_vocab["<unk>"])
        )
        hardware_tensor = torch.tensor(hardware_id, dtype=torch.long)

        # Use already scaled program features
        if scale:
            program_feature_value = torch.tensor(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocess NeuroOpt hybrid training data."
    )
    parser.add_argument(
        "--input_json",
        type=str,
    ...
    if total_sequences > 0:
        print(
            f"Percentage of sequences containing 'none': {(total_none_sequences / total_sequences) * 100:.2f}%"
        )
    print(
        "You can now use these artifacts and the processed_samples (if returned) for model training."
    )
