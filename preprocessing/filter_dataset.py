...
exec_stats_before = full_df.groupby(group_cols)["execution_time"].agg(
    ["min", "max", "mean"]
)
...
print(
    f"Before filtering -> execution_time mean: {full_df['execution_time'].mean():.6f}"
)
print(
    f"Before filtering -> execution_time variance: {full_df['execution_time'].var():.6f}"
)
...
for hw, count in hw_counts.items():
    if count < MIN_HW_COUNT:
        ...
        candidates = (
            full_df[full_df["hardware_signature"] == hw]
...
filtered_df = filtered_df.drop_duplicates(
    subset=["program", "hardware_signature", "pass_sequence"]
)

counts_after = filtered_df.groupby(group_cols).size()
if not counts_after.empty:
    exec_stats_after = filtered_df.groupby(group_cols)["execution_time"].agg(
        ["min", "max", "mean"]
    )
    ...
    print(
        f"After filtering -> mean(min exec_time): {exec_stats_after['min'].mean():.6f}"
    )
    print(
        f"After filtering -> mean(max exec_time): {exec_stats_after['max'].mean():.6f}"
    )
    print(
        f"After filtering -> execution_time mean: {filtered_df['execution_time'].mean():.6f}"
    )
    print(
        f"After filtering -> execution_time variance: {filtered_df['execution_time'].var():.6f}"
    )
else:
